...
{
    if(!((secp256k1.verify(fingerprint, signature0, address0) && secp256k1.verify(fingerprint, signature1, address1))))
    {
...
function channelDoesNotExist (state, channelId) {
    if((state.channels[channelId] !== null) && (state.channels[channelId] !== undefined))
    {
...
function channelIsSettled (state, channelId) {
    if(state.channels[channelId].settlingPeriodStarted && 
      state.channels[channelId].sequenceNumber < state.channels[channelId].settlingPeriodEnd)
    {
...
app.use((state, tx, chainInfo) => {
  if (tx.type === "newChannel") {
    ...
    fingerprint = sha256("new", tx.channelId, tx.address0, tx.address1, tx.balance0, tx.balance1, tx.settlingPeriodLength);
    signedByBoth(fingerprint, tx.signature0. tx.signature1, tx.address0, tx.address1)
  ...
  if (tx.type === "updateState") {
    ...
    fingerprint = sha256("update", tx.channelId, tx.sequenceNumber, tx.balance0, tx.balance1, tx.hashlocks);
    signedByBoth(fingerprint, tx.signature0. tx.signature1, channels[tx.channelId].address0, channels[tx.channelId].address1);
  ...
  if (tx.type === "startSettlingPeriod") {
    ...
    signedByOne(fingerprint, tx.signature, channels[tx.channelId].address0, channels[tx.channelId].address1)
...
function closeChannelInternal(state, channelId) {
  ...
  let balances = applyHashlockAdjustment(channelId, state.channels[channelId].balance0,
    state.channels[channelId].balance1, adjustment).balance0;
